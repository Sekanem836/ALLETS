...
def engineer_features(df, feature_names, tfidf_vectorizer, pca_transformer, embedding_model, tokenizer):
    ...
    try:
        ...
        def safe_get_embedding(text, tokenizer, embedding_model):
            try:
                return get_embedding(text, tokenizer, embedding_model)
            except Exception as e:
        ...
        df['adjuster_embedding'] = df['Adjuster_Notes'].apply(lambda x: safe_get_embedding(x, tokenizer, embedding_model))
    ...
    for col in feature_names:
        if col not in df.columns:
            ...
            # A more robust approach would be to save the dtypes during training and use them here.
            # Check if the column is expected to be numerical (based on prefixes or common patterns)
            if col.startswith('tfidf_') or col.startswith('embed_pca_') or '_Flag' in col or '_on_' in col or '_Days' in col or '_Count' in col or '_Ratio' in col or 'Centrality' in col or 'Sentiment_Score' in col:
                 df[col] = 0.0
            # Check if the column is a one-hot encoded categorical feature (based on presence in original categorical features and their unique values)
            # This is a bit tricky without the original categorical feature names list here.
            # A simpler heuristic for now is to check for presence in a hardcoded list or based on a pattern.
            # Let's assume any column not identified as numerical/binary is potentially categorical and fill with 'missing'.
            # This relies on the preprocessor to handle 'missing' appropriately.
            elif any(cat_col + '_' in col for cat_col in ['Location', 'Policy_Type', 'Claim_Type', 'Incident_Type', 'Customer_Gender', 'Customer_Occupation']): # Common categorical prefixes
                 df[col] = 0.0 # One-hot encoded columns are typically numerical (0 or 1)
            else: # Default catch-all, assuming numerical if not explicitly identified otherwise
                 df[col] = 0.0
...
def preprocess_data(df, preprocessor, feature_names):
    ...
    st.write("DataFrame columns after adding missing columns:", df.columns.tolist())

    # Ensure the order of columns matches the training data
    # Only keep columns that are in feature_names and in the correct order
    try:
        df_aligned = df[feature_names]
    except KeyError as e:
        st.error(f"Error aligning columns: {e}. One or more expected features are missing or misspelled after engineering.")
        st.write("Expected features:", feature_names)
        st.write("Features available in DataFrame:", df.columns.tolist())
        st.stop()
        return None
...
if option == "Manual Input":
    ...
    if st.button("Predict Fraud"):
        # Engineer features
        engineered_df = engineer_features(input_df.copy(), feature_names, tfidf_vectorizer, pca_transformer, embedding_model, tokenizer)
    ...
    if uploaded_file is not None:
        try:
            ...
            engineered_csv_df = engineer_features(csv_df.copy(), feature_names, tfidf_vectorizer, pca_transformer, embedding_model, tokenizer)
            ...
            if processed_csv_data is not None:
                ...
                colors = ['#ff9999','#66b3c9'] # Red for Fraudulent, Green for Legitimate
